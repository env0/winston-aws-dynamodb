  ...
  while (entryIndex < Math.min(logEvents.length, lib.MAX_BATCH_ITEM_NUM)) {
    ...
    if (evSize > LIMITS.MAX_EVENT_MSG_SIZE_BYTES) {
      ...
      const msgTooBigErr = new Error(
        'Message Truncated because it exceeds the DynamoDB size limit'
      );
...
const buildPayload = (
  ...
  const entries = additionalAttributesSchema
    ? Object.entries(additionalAttributesSchema)
    : [];
  return {
    RequestItems: {
...
lib.upload = function (aws, tableName, streamName, logEvents, options, cb) {
  ...
  const payload = buildPayload(
    ...
    streamName,
    eventsToSend,
    options.dynamoDbAdditionalAttributesSchema
  );
