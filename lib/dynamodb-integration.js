...
const isDynamoDbError = (err, data) => err || !isEmpty(data.UnprocessedItems);
  ...
  dynamodbClient.send(new BatchWriteItemCommand(payload), function (err, data) {
    ...
    if (isDynamoDbError(err, data) && times > 0) {
      ...
      if (!isEmpty(data.UnprocessedItems)) {
        retrySubmit(dynamodbClient, {RequestItems: data.UnprocessedItems}, cb, 3)
      } else {
        retrySubmit(dynamodbClient, payload, cb, 3)
      }
    } else {
      cb()
    }
