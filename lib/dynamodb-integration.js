...
const LIMITS = {
  ...
  MAX_CONCURRENT_BATCH_WRITING: 25   // Write a maximum of 25 batches to dynamo concurrently
}
    ...
    logsQueue.add(() => {
      return new Promise((resolve) => {
        dynamodbClient.send(new BatchWriteItemCommand(payload), function (err, data) {
          ...
          if (err || !isEmpty(data.UnprocessedItems)) {
              debug('error during batchWriteItem', err, true)

              if (!isEmpty(data.UnprocessedItems)) {
                retrySubmit(dynamodbClient, {RequestItems: data.UnprocessedItems}, 3, cb)
              } else {
...
function retrySubmit(dynamodbClient, payload, times, cb) {
    ...
    logsQueue.add(() => {
      return new Promise((resolve) => {
        dynamodbClient.send(new BatchWriteItemCommand(payload), function(err, data) {
          debug('sent to aws, err: ', err, ' data: ', data, 'times: ', times);
            if ((err || !isEmpty(data.UnprocessedItems)) && times > 0) {
              if(!isEmpty(data.UnprocessedItems)) {
                retrySubmit(dynamodbClient, { RequestItems: data.UnprocessedItems }, times - 1, cb)
              } else {
