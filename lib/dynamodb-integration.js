...
lib.upload = function(dynamodbClient, tableName, streamName, logEvents, options, cb) {
    ...
    dynamodbClient.send(new BatchWriteItemCommand(payload), function (data, err) {
        debug('sent to aws, err: ', err, ' data: ', data)
        console.log('something for yossi');
        if (err) {
...
function retrySubmit(dynamodbClient, payload, times, cb) {
    ...
    dynamodbClient.send(new BatchWriteItemCommand(payload), function(_, err) {
        if (err && times > 0) {
