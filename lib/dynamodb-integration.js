...
const buildPayload = (tableName, streamName, events, extraFieldsSchema) => {
    var entries = extraFieldsSchema ? Object.entries(extraFieldsSchema) : [];
    return {
        RequestItems: {
            [tableName]: events.map(event => {
                    const extraValues = {};
                    if (extraFieldsSchema) {
                        entries.forEach(([key, dynamoValueType]) => {
                            if (!isUndefined(event[key])) {
                                extraValues[key] = {
                                    [dynamoValueType]: events[key]
                                };
                            }
                        });
                    }
...
lib.upload = function (aws, tableName, streamName, logEvents, options, cb) {
    ...
    const payload = buildPayload(tableName, streamName, eventsToSend, options.extraFieldsSchema);
